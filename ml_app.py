...
app = Flask(__name__)
basepath = os.path.abspath(os.getcwd())  # important for server to find models folder
...
def my_form_post():
    ...
    with open('comment.csv','a') as file:
        file.write(f"""{text},{dict_preds['pred_toxic']},{dict_preds['pred_severe_toxic']},{dict_preds['pred_identity_hate']},{dict_preds['pred_insult']},{dict_preds['pred_obscene']},{dict_preds['pred_threat']}""")
